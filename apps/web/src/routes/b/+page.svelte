<script lang="ts">
  ...
  const rawBookData$ = bookId$.pipe(
    switchMap(async (id) => {
      ...
      try {
        ...
        await hydrateLinkedCloudReaderProgress(bookData.id).catch((error) => {
          logger.warn(`Cloud progress restore failed: ${error instanceof Error ? error.message : String(error)}`);
        });
  ...
  async function completeBook() {
    ...
    try {
      ...
      if (bookmarkManager) {
        ...
        void saveLinkedCloudReaderProgress($rawBookData$.id, data).catch(() => undefined);
  ...
  async function bookmarkPage() {
    ...
    bookmarkData = Promise.resolve(data);
    void saveLinkedCloudReaderProgress(bookId, data).catch(() => undefined);
