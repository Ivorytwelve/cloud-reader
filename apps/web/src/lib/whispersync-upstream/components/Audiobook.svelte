	...
	async function onResetBook() {
		...
		try {
			...
			document.dispatchEvent(
				new CustomEvent('ttu-cloud:alignment-reset', { detail: { localBookId: $bookData$.id } }),
			);
		...
		<div class="flex justify-between w-full header-icons">
			...
			<Popover placement="bottom">
				...
				{#if $bookMatched$.matchedBy}
					...
					{/if}
					{#if $bookMatched$.totalLines}
						<div class="m-t-xs">Match rate: {((($bookMatched$.matchedLines || 0) / $bookMatched$.totalLines) * 100).toFixed(1)}%</div>
						<div>{$bookMatched$.matchedLines || 0} / {$bookMatched$.totalLines} lines</div>
						{#if $bookMatched$.matchedSource}<div>Source: {$bookMatched$.matchedSource === 'cloud-auto' ? 'Automatic cloud match' : $bookMatched$.matchedSource}</div>{/if}
					{/if}
				{:else}
			...
			<Popover placement="bottom" bind:this={audioFilePopover}>
				...
				<div>{$currentAudioFile$?.name || $currentRemoteAudioFileName$ || 'None selected'}</div>
				<button title={changeAudioTitle} disabled={!!$exportCancelController$} on:click={onSetAudioFile}>
					<Icon path={$currentAudioFile$ || $currentRemoteAudioFileName$ ? mdiFileArrowLeftRightOutline : mdiFilePlusOutline} />
				</button>
...
<div class="flex flex-col flex-1 min-h-0">
	...
	{#if $currentAudioSourceUrl$}
		...
		<Player
			...
			on:loaded={() => subtitleListElement?.onAudioReady()}
			on:error={() => {
