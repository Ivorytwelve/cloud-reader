		...
		try {
			...
			if (metadataElement instanceof HTMLElement) {
				$bookMatched$ = {
					...
					matchedLines: Number.parseInt(metadataElement.dataset.ttuWhispersyncMatchedLines || '0', 10) || undefined,
					totalLines: Number.parseInt(metadataElement.dataset.ttuWhispersyncTotalLines || '0', 10) || undefined,
					diffLines: Number.parseInt(metadataElement.dataset.ttuWhispersyncDiffLines || '0', 10) || undefined,
					matchedSource: metadataElement.dataset.ttuWhispersyncMatchedSource || undefined,
				};
			}
...
>
	...
	<div
		class="flex flex-col flex-1 min-h-0 p-x-b p-y-s"
	...
	>
		{#if !loadError && $bookData$?.elementHtml && $extensionData$?.title}
			<div class="flex justify-between w-full tabs m-b-s shrink-0">
				<button
			...
			<div
				class="flex-col flex-1 min-h-0 m-b-s"
