<script lang="ts">
	...
	async function onMatchSubtitles() {
		...
		try {
			...
			matchedLineCount = matchedSubtitles;
			totalLineCount = subtitles.length;
			diffLineCount = subtitleDiffDetails.length;
			...
			if (bookHTML.firstElementChild instanceof HTMLElement) {
				...
				bookHTML.firstElementChild.dataset.ttuWhispersyncMatchedSource = 'manual';
				bookHTML.firstElementChild.dataset.ttuWhispersyncMatchedLines = `${matchedSubtitles}`;
				bookHTML.firstElementChild.dataset.ttuWhispersyncTotalLines = `${subtitles.length}`;
	...
	async function onSaveMatch() {
		...
		try {
			...
			// Native Ttsu cloud integration can persist a manual replacement alignment
			// before the reload. Userscript installs simply have no listener.
			const pendingCloudSaves: Promise<unknown>[] = [];
			document.dispatchEvent(
				new CustomEvent('ttu-cloud:alignment-saved', {
					detail: {
						localBookId: $bookData$.id,
						elementHtml: newData.elementHtml,
						match: {
							matchedBy: $currentSubtitleFile$?.name || '',
							matchedOn: Number.parseInt(bookHTML!.firstElementChild?.getAttribute('data-ttu-whispersync-matched-on') || '0', 10) || Date.now(),
							matchedLines: matchedLineCount,
							totalLines: totalLineCount,
							diffLines: diffLineCount,
							rate: totalLineCount ? matchedLineCount / totalLineCount : 0,
						},
						waitUntil: (promise: Promise<unknown>) => pendingCloudSaves.push(promise),
					},
				}),
			);
			if (pendingCloudSaves.length) await Promise.all(pendingCloudSaves);
