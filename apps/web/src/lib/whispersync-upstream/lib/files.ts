...
export async function setAudioContext(
	...
	if (newAudioFile) currentRemoteAudioFileName$.set('');
	...
	if (!newAudioFile) {
		// Remote/cloud audio has no local File object. Keep time fallback enabled so
		// the subtitle pane can follow the restored playback position before the
		// browser's text track emits its first cue event.
		activeSubtitle$.set({ previous: '', current: '', useTimeFallback: true });
	}
