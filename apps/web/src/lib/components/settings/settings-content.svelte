<script lang="ts">
  ...
  import CloudSettings from '$lib/cloud/cloud-settings.svelte';
  ...
  export let audiobookCountShortPauses: boolean;
  ...
  $: if (activeSettings === 'Statistics' && !$storageSources$) {
    database
      .getStorageSources()
      .then((storageSources) => {
...
<div class="grid grid-cols-1 items-center sm:grid-cols-2 sm:gap-6 lg:md:gap-8 lg:grid-cols-3">
  ...
  {:else if activeSettings === 'Cloud'}
    ...
    <SettingsItemGroup
      ...
      tooltip="Ask the browser to protect this site's local cache and settings from automatic storage eviction. Cloud files remain canonical."
    >
      <div class="flex items-center">
      ...
      {#if audiobookCountShortPauses}
        ...
        >
          <input
            ...
            min="1"
            max="300"
            bind:value={audiobookShortPauseSeconds}
            on:blur={() => {
              const value = Number(audiobookShortPauseSeconds);
              audiobookShortPauseSeconds = Number.isFinite(value)
